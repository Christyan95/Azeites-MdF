...
class TelaProdutoAtualizar : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        bundle?.apply {
            val produto: Produto? = getParcelable("produto")
            setContent {
                minhaTelaProdutoAtualizar(produto = produto)
            }
        }
    }
}
...
@Composable
fun minhaTelaProdutoAtualizar(modifier: Modifier = Modifier, produto: Produto?) {

    val referencia = Firebase.database.getReference("/produto")

    var text_id_produto by remember { mutableStateOf(TextFieldValue(produto!!.id_produto)) }
    var text_descricao by remember { mutableStateOf(TextFieldValue(produto!!.descricao)) }
    var text_valor by remember { mutableStateOf(TextFieldValue(produto!!.valor.toString())) }
    ...
    if (carregamento) {
        ...
        Column(
            ...
            Text("  - Atualizar Produtos | Azeites MdF - ",
                fontSize = 25.sp,
            ...
            Button(
                onClick = {
                    ...
                    produto!!.id_produto = text_id_produto.text
                    produto!!.descricao = text_descricao.text
                    produto!!.valor = text_valor.text.toFloat()
                ...
                Text(text = "Atualizar Produto", color = Color(android.graphics.Color.parseColor("#1C2000")), fontWeight = FontWeight.Bold)
            }
        }
    }
}
